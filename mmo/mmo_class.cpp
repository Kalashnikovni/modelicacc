...
MMO_Class::MMO_Class(Class &c) : variables_()
{
  ...
  foreach_(CompElement ce, c.composition().comp_elem())
  {
    if (is<EquationSection>(ce)) {
      ...
      foreach_(Element e, pub) insertElement(e);
    } else if (is<Protected>(ce)) {
      Protected piv = boost::get<Protected>(ce);
      foreach_(Element e, piv) insertElement(e);
    }
  }
}
...
std::ostream &operator<<(std::ostream &out, const MMO_Class &c)
{
  ...
  foreach_(Option<ClassPrefix> cp, c.prefixes()) out << classPrefix(cp);
  out << c.name() << "\n";
  BEGIN_BLOCK;

  foreach_(Import n, c.imports()) out << INDENT << n << ";\n";

  foreach_(Extends n, c.extends()) out << INDENT << n << ";\n";

  foreach_(Name n, c.types())
  {
    Option<Type::Type> t = tipos[n];
    if (t) out << INDENT << (t.get()) << std::endl;
  }
  ...
  foreach_(Name n, c.variables())
  {
    ...
    out << INDENT;
    foreach_(Option<TypePrefix> tp, vinfo.prefixes()) out << typePrefix(tp);
    out << vinfo.type() << " " << n;
    if (vinfo.indices()) {
      ExpList el = vinfo.indices().get();
      int l = el.size(), i = 0;
      out << "[";
      foreach_(Expression e, el) out << e << (++i < l ? ", " : "");
      out << "]";
    }
    if (vinfo.modification()) out << vinfo.modification().get();
    out << ";\n";
  }
  ...
  if (c.external()) {
    out << INDENT << " external";
    if (c.language()) out << " " << c.language().get() << " ";
    if (c.external()) {
      External e = c.external().get();
      if (e.comp_ref()) out << e.comp_ref().get() << " =";
      out << " " << e.fun() << "(";
      unsigned int i = 0;
      foreach_(OptExp oe, e.args())
      {
        if (oe) out << oe.get() << (++i < e.args().size() ? "," : "");
      }
      out << ")";
      if (c.external_annot()) out << c.external_annot().get();
    }
    out << ";\n";
  }
  if (c.annotation()) out << c.annotation().get() << ";\n";
...
{
  foreach_(Option<ClassPrefix> cp, prefixes()) if (cp && (cp.get() == connector)) return true;
  return false;
}
