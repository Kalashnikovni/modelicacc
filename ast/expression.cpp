...
{
  foreach_(boost::optional<char> c, ss) val_ += (c ? std::string(1, c.get()) : "\\\"");
}
bool String::operator==(const String& other) const { return (other.val() == val()); }
    ...
    : cond_(a), then_(b), elseexp_(c)
{
  ...
  foreach_(P f, elseif) elseif_.push_back(ExpPair(get<0>(f), get<1>(f)));
};
...
std::ostream& operator<<(std::ostream& out, const IfExp& f)  // output
{
  ...
  foreach_(ExpPair p, f.elseif()) out << " elseif " << get<0>(p) << " then " << get<1>(p);
  out << " else " << f.elseexp();
  return out;
}
...
bool Range::operator==(const Range& other) const
{
  if (other.step()) {
    ...
    if (!(other.step().get() == step().get())) return false;
  } else {
    if (step()) return false;
  }
...
bool FunctionExp::operator==(const FunctionExp& other) const { return other.name() == name() && other.args() == args(); }
...
long printAsCActive(std::ios_base& s) { return printAsC(s); }

void setCFlag(std::ios_base& s, long n) { printAsC(s) = n; }
